...
namespace Tampa.InterfaceCompiler
{
    class JavaCompiler: ICompiler
    {
        ...
        private void WriteMain(string className)
        {
            Write("public static void main(String[] args) {", 1);
            Write("SwingUtilities.invokeLater(new Runnable() {", 2);
            Write("public void run() {", 3);
            Write(String.Format("JFrame frame = (new {0}()).GetCanvas();", className), 4); // GetCanvas is the method generated by the CanvasWriter
            Write("frame.setVisible(true);", 4);
            Write("}", 3);
            Write("});", 2);
            WriteEnd(1);
        }
